...
### Relationship Managers
RM’s are responsible for handling sales calls and service enquiries. RM’s are given a skill score based on the duration of previous calls and their profile. RM’s are concerned with losing points in their skill score due to an inability to capture a customer’s attention, being uniformed about customer profiles which leads to longer phone calls. To eliminate this issue, we suggest the CMC leverages the Profiler Tool, which matches customer profiles with RM profiles and assists RM’s in understanding customer profiles. To leverage the Profiler Tool, we have brainstormed the following ideas

* RM’s should be trained to know when to abandon the sales call (i.e. uninterested customer)
* RM’s should be trained to understand different customer profiles
* RM’s should proactively check the Automatic Call Distributor for customers that fit their profile during downtime
* RM’s should be well informed about all holiday packages and focus on selling the most compatible package to a customer
* Train RM’s to deal with aggressive or uncooperative customers

### Inbound Callers
Inbound callers are customers who contact the travel agency via the CMC. Often, inbound calls are directed to the Interactive Voice Response (IVR) unit. This process can be time consuming and frustrating for customers. Inbound callers can be dissatisfied with the RM, who may be unable to resolve their enquiries. Moreover, customers are assigned a score based on their likelihood to purchase products. These scores are generated by the system and may present some oversight. The brainstormed ideas for these problems include:
...
* RM’s should contribute to the customer score allocation process by scoring customers based on parameters such as high budget. This may provide the opportunity for RM’s to prioritise customers with a higher budget. Though, bias, and unfair prioritisation must be accounted for 

### Outbound Call Customers
Outbound call customers are customers that receive calls from RM’s. RM’s are provided scripts and sales guidelines for all outbound calls. Outbound customers have reported that they receive unnecessary and untimely calls and are unwilling to pick up calls from an unknown number. Outbound customers may not want to hear about holiday packages or recommended packages may not be viable for their budget and lifestyle. The following ideas have been generated to solve these issues:
...
* Ensure RM’s are highly knowledgeable of the socioeconomic status of their outbound customers so that the best possible package can be offered
